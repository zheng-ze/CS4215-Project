...
export class RustLiteVirtualMachine implements VirtualMachine<SUPPORTED_TYPES> {
  ...
  reset(): void {
    // Reset pointers instead of creating new instances
    this.stack.sp = 0;
    this.stack.fp = 0;
    this.heap.free = 0;
    this.pc = 0;
  ...
  microcode: VirtualMachineMicrocode = {
    ...
    [instruction_type.RESET]: (instr: instruction) => {
      const returnValue = this.stack.peek();  // Save return value
      this.stack.popFrame();  // Remove current frame
      const returnAddr = this.stack.pop();  // Get return address
      this.pc = returnAddr;  // Restore return address
      this.stack.push(returnValue);  // Push return value back
    },
  ...
  private apply_binop(
    ...
    // Convert numeric 0/1 to boolean for boolean operations
    if (op === "&&" || op === "||") {
      return operation(left === 0 ? false : true, right === 0 ? false : true) ? 1 : 0;
    }

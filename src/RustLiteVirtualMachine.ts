  ...
  microcode: VirtualMachineMicrocode = {
    ...
    [instruction_type.ENTER_SCOPE]: (instr: instruction) => {
      ...
      if (this.stack.getFrameCount() > 0) {
        ...
        console.log(
          `Propagating return address ${returnAddr} from parent frame to new scope frame`
        );
      }
      ...
      try {
        ...
        if (ld.pos.first === 2) {
          ...
          if (currentFrameIndex < 0) {
            throw new Error(
              `Cannot access frame level ${ld.pos.first} when no frames exist`
            );
          }
          ...
          if (
            ld.pos.second >= currentFrame.frameSize &&
            currentFrameIndex > 0
          ) {
            const parentFrameIndex = currentFrameIndex - 1;
            const value = this.stack.getLocalFromFrame(
              parentFrameIndex,
              ld.pos.second
            );
            this.stack.push(value);
            console.log(
              `Loaded value from parent frame ${parentFrameIndex}, offset ${ld.pos.second}: ${value}`
            );
          } else {
            const value = this.stack.getLocalFromFrame(
              currentFrameIndex,
            ...
            this.stack.push(value);
            console.log(
              `Loaded value from frame level ${ld.pos.first} (frame index ${currentFrameIndex}), offset ${ld.pos.second}: ${value}`
            );
          }
          ...
          if (frameIndex < 0) {
            throw new Error(
              `Cannot access frame level ${
                ld.pos.first
              } when only ${this.stack.getFrameCount()} frames exist`
            );
          }
          ...
          this.stack.push(value);
          console.log(
            `Loaded value from frame level ${ld.pos.first} (frame index ${frameIndex}), offset ${ld.pos.second}: ${value}`
          );
        } else {
          throw new Error(
            `Accessing variables from frame level ${ld.pos.first} not yet implemented`
          );
        }
      } catch (error: any) {
        console.error(
          `Error accessing variable at position ${ld.pos.first}.${ld.pos.second}: ${error.message}`
        );
        throw error;
      }
    },
    ...
    [instruction_type.ASSIGN]: (instr: instruction) => {
      ...
      try {
        ...
        if (assign.pos.first === 2) {
          ...
          if (frameIndex < 0) {
            throw new Error(
              `Cannot assign to frame level ${assign.pos.first} when no frames exist`
            );
          }

          this.stack.setLocalInFrame(frameIndex, assign.pos.second, value);
          console.log(
            `Assigned value ${value} to variable at frame ${frameIndex}, offset ${assign.pos.second}`
          );
          ...
          throw new Error(
            `Assigning to variables at frame level ${assign.pos.first} not yet implemented`
          );
        }
      } catch (error: any) {
        console.error(
          `Error assigning to variable at position ${assign.pos.first}.${assign.pos.second}: ${error.message}`
        );
        throw error;
      }
    },
    ...
    [instruction_type.LDF]: (instr: instruction) => {
      ...
      console.log(
        `LDF: Loaded function at address ${ldf.addr} with arity ${ldf.arity}`
      );
    },
      ...
      if (returnAddr === undefined) {
        ...
        for (let i = this.stack.getFrameCount() - 2; i >= 0; i--) {
          ...
          if (frame.returnAddress !== undefined) {
            ...
            console.log(
              `Found return address ${foundReturnAddr} in parent frame ${i}`
            );
            break;
          }
        }
        ...
        console.log(
          `RESET: Returning to address ${foundReturnAddr} with value ${returnValue}`
        );

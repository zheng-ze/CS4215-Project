...
interface StackFrame {
  ...
  environment?: number;   // Environment pointer for lexical scoping
...
export class RustLiteStack {
  ...
  private framePointer: number = 0; // Points to the current frame
  private frames: StackFrame[] = []; // Metadata about frames
  private scopeDepth: number = 0;    // Track lexical scope depth for lifetimes
  ...
  public pushFrame(frameSize: number, returnAddress?: number): void {
    console.log(
      `Pushing frame with size: ${frameSize}, Stack pointer: ${this.stackPointer}, Frame pointer: ${this.framePointer}, Return address: ${returnAddress}`
    );
    
    const actualFrameSize = Math.max(frameSize, 1);

    // Check if we have enough space for the new frame
    if (this.stackPointer + actualFrameSize >= max_words) {
      console.error(`Stack overflow: Cannot allocate frame of size ${actualFrameSize} at position ${this.stackPointer} (max: ${max_words})`);
    ...
    for (let i = 0; i < actualFrameSize; i++) {
      this.data.setFloat64((this.stackPointer + i) * word_size, 0, true);
    }
  ...
  public setFrameEnvironment(env: number): void {
    if (this.frames.length === 0) {
      throw new Error("No active frame");
    }
  ...
  public getFrameEnvironment(frameIndex: number): number | undefined {
  ...
  public getLocalFromFrame(frameIndex: number, offset: number): SUPPORTED_TYPES {
    if (frameIndex < 0 || frameIndex >= this.frames.length) {

...
class RustLiteTypeChecker {
    ...
    visitDeclareStmt(ctx) {
        ...
        if (exprType === "void")
            throw new Error(`Invalid variable declaration: initializer cannot be void`);
        if (typeof exprType === "object" &&
            exprType.kind === "vec" &&
            exprType.elementType === undefined) {
            if (!type)
                throw new Error("Type annotation is required for empty vector initialization");
        ...
        if (type && exprType && type !== exprType) {
            ...
            if (typeof type === "object" &&
                ...
                if (type.elementType !== exprType.elementType) {
                    if (!(exprType.elementType === "u64 | i64" &&
                        (type.elementType === "u64" || type.elementType === "i64"))) {
                        throw new Error(`Type mismatch: expected ${JSON.stringify(type)} but got ${JSON.stringify(exprType)}`);
                    }
                }
            }
        }
    ...
    visitFnDeclareStmt(ctx) {
        ...
        for (const [name, type] of params) {
            if (this.current_frame.mappings.has(name)) {
                throw new Error(`Identifier ${name} is bound more than once in this parameter list`);
            }
            this.current_frame.mappings.set(name, type);
        }
        ...
        if (bodyReturnType !== returnType) {
            throw new Error(`Return type mismatch: expected ${returnType}, got ${bodyReturnType}`);
        }
    ...
    getTypeOfExpr(ctx) {
        ...
        if (primary)
            return this.getTypeOfToken(primary);
        const inner = ctx._inner;
        if (inner)
            return this.getTypeOfExpr(inner);
        const arithExpr = ctx.arithExpr();
        if (arithExpr)
            return this.getTypeOfArithExpr(arithExpr);
        const boolExpr = ctx.logicExpr();
        if (boolExpr)
            return this.getTypeOfLogicExpr(boolExpr);
        const vectorExpr = ctx.vectorExpr();
        if (vectorExpr)
            return this.getTypeOfVectorExpr(vectorExpr);
        const fnCall = ctx.fnCall();
        if (fnCall)
            return this.getTypeOfFnCall(fnCall);
        throw new Error("Invalid expression");
    }
    getTypeOfArithExpr(ctx) {
        ...
        if (left && right && op) {
            ...
            throw new Error(`Mismatched types for ${opType}: ${leftType} and ${rightType}`);
        }
        // If we reach here, it means we have an invalid expression
        throw new Error("Invalid arithmetic expression");
    }
    getTypeOfLogicExpr(ctx) {
        ...
        const inner = ctx._inner;
        if (inner)
            return this.getTypeOfLogicExpr(inner);
        const left = ctx._left;
        const right = ctx._right;
        const op = ctx._op;
        // Logical not
    ...
    getTypeOfFnCall(ctx) {
        ...
        if (args.length === 0 && fnType.paramTypes.length > 0)
            throw new Error("");
        if (args.length !== fnType.paramTypes.length)
            throw new Error(`Function ${fnName} expects ${fnType.paramTypes.length} arguments but got ${args.length}`);
    ...
    getReturnTypeOfStmt(ctx) {
        ...
        if (returnStmtCtx)
            return this.getReturnTypeOfReturnStmt(returnStmtCtx);
        if (condStmtCtx)
            return this.getReturnTypeOfCondStmt(condStmtCtx);
        if (whileStmtCtx)
            return this.getReturnTypeOfWhileStmt(whileStmtCtx);
        if (blockCtx)
            return this.wrapWithFrameAndEvaluate(this.getBodyReturnType.bind(this, blockCtx));
        return "void";
    }
    getReturnTypeOfCondStmt(ctx) {
        console.log("Getting return type of conditional statement");
        if (!ctx)
            return "void";
        const blockCtx = ctx.block();
        const blockTypes = blockCtx.map((block) => this.wrapWithFrameAndEvaluate(this.getBodyReturnType.bind(this, block)));
    ...
    getReturnTypeOfReturnStmt(ctx) {
        console.log("Getting return type of return statement");
        if (!ctx)
            throw new Error("Return statement context is null");
        const expr = ctx.expr();
        if (!expr)
            return "void";
    ...
    getTypeOfType(ctx) {
        ...
        if (!ctx)
            throw new Error("Type context is null");
    ...
    getTypeOfToken(token) {
        ...
        switch (token.type) {
            case RustLiteParser_1.RustLiteParser.BOOL:
                return "bool";
            case RustLiteParser_1.RustLiteParser.INT:
                ...
                if (number < 2 ** 63 - 1)
                    return "u64 | i64";
                return "u64";
            case RustLiteParser_1.RustLiteParser.IDENTIFIER:
                const name = token.text;
                if (!name || name.length === 0) {
                    throw new Error(`Invalid identifier: ${name}`);
                }
                return this.getTypeOfIdentifier(name);
            case RustLiteParser_1.RustLiteParser.U64_TYPE:
                return "u64";
            case RustLiteParser_1.RustLiteParser.I64_TYPE:
                return "i64";
        }
        throw new Error(`Unknown token type: ${token.text}`);
    }
    getTypeOfIdentifier(identifier) {
        ...
        let currentFrame = this.current_frame;
        while (currentFrame) {
    ...
    createAndAddFunctionFrame(name) {
...
exports.RustLiteTypeChecker = RustLiteTypeChecker;
